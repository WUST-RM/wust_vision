...
public:
  ...
  struct DistMapNode {
    DistMapNode(const LineRegion &region,
                const std::vector<HeightMapNode> h_nodes)
  ...
  bool updateMap(const LineRegion &d_region, const LineRegion &h_region,
                 const double pitch_offset, const double yaw_offset);
...
=======
  ...
  bool updateMapFlow(const std::vector<OffsetEntry>& entries) {
    for (const auto& e : entries) {
      if (!updateMap(LineRegion(e.d_min, e.d_max),
