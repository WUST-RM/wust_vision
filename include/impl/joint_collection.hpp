...
template <typename T, typename... Ts> struct unique : std::type_identity<T> {};
...
template <typename T, typename U> struct has_type;
...
inline bool
get_joint_modified(const internal::has_modification_getter auto &joint) {
...
inline void set_joint_modified(internal::has_modification_setter auto &joint,
                               bool value) {
...
template <internal::is_non_root_link... ChildLinkTs> class JointCollection {
public:
  using TupleT = internal::unique_tuple<Joint<ChildLinkTs>...>;

  JointCollection() requires(
      std::is_same_v<TupleT, std::tuple<Joint<ChildLinkTs>...>>) = default;

  [[deprecated("Link types passed in as template parameter is "
               "duplicated.")]] JointCollection() = default;

  template <typename ChildT> struct contains_joint {
    static constexpr bool value =
        internal::has_type<Joint<ChildT>, TupleT>::value;
  };
  ...
  auto &get_joint() { return std::get<fast_tf::Joint<To>>(collection_); }

  template <internal::is_link From, internal::is_link To>
  requires internal::has_joint<From, To>
  const auto &get_joint() const {
    return std::get<fast_tf::Joint<To>>(collection_);
  }

  template <internal::is_link From, internal::is_link To, typename... ArgTs>
  requires(internal::has_setter_joint<From, To, ArgTs...>
               &&contains_joint_v<To>) void set_transform(ArgTs &&...value) {
  ...
  template <internal::is_link From, internal::is_link To,
            internal::is_transform TransformT>
  requires(!internal::has_setter_joint<From, To, TransformT> &&
           internal::has_transform_joint<From, To> &&
           contains_joint_v<To>) void set_transform(TransformT &&transform) {
    auto &joint = get_joint<From, To>();
  ...
  template <internal::is_link From, internal::is_link To,
            internal::is_translation TranslationT>
  requires(
      !internal::has_setter_joint<From, To, TranslationT> &&
      internal::has_translation_joint<From, To> &&
      contains_joint_v<To>) void set_transform(TranslationT &&translation) {
    auto &joint = get_joint<From, To>();
  ...
  template <internal::is_link From, internal::is_link To,
            internal::is_rotation RotationT>
  requires(!internal::has_setter_joint<From, To, RotationT> &&
           internal::has_rotation_joint<From, To> &&
           contains_joint_v<To>) void set_transform(RotationT &&rotation) {
    auto &joint = get_joint<From, To>();
  ...
  template <internal::is_link From, internal::is_link To, typename... ArgTs>
  requires(internal::has_stateful_joint<From, To, ArgTs...>
               &&contains_joint_v<To>) void set_state(ArgTs &&...value) {
    auto &joint = get_joint<From, To>();
  ...
  template <internal::is_link From, internal::is_link To>
  requires(internal::has_getter_joint<From, To>
               &&contains_joint_v<To>) auto get_transform() const {
    return get_joint<From, To>().get_transform();
  }

  template <internal::is_link From, internal::is_link To>
  requires(!internal::has_getter_joint<From, To> &&
           internal::has_joint<From, To> && contains_joint_v<To>) const
      auto &get_transform() const {
    return get_joint<From, To>().transform;
  }

  template <typename JointCollectionTo> auto as_reference();
...
template <typename T>
concept is_joint_collection = requires(T collection) {
  test_joint_collection(collection);
};

template <internal::is_joint_collection FromCollection,
          internal::is_non_root_link... ToCollectionChildLinkTs>
requires(
    FromCollection::template contains_joint_v<ToCollectionChildLinkTs>
        &&...) void test_joint_collection_castable(const FromCollection &,
                                                   const JointCollection<
                                                       ToCollectionChildLinkTs...>
                                                       &);

template <typename From, typename To>
concept is_joint_collection_castable = requires(From from, To to) {
  test_joint_collection_castable(from, to);
};

} // namespace internal

template <internal::is_joint_collection JointCollectionFrom,
          internal::is_joint_collection JointCollectionTo>
requires internal::is_joint_collection_castable<JointCollectionFrom,
                                                JointCollectionTo>
class JointCollectionReference {
public:
  explicit JointCollectionReference(JointCollectionFrom &ref) : ref_(ref) {}
  ...
  template <internal::is_link From, internal::is_link To, typename... ArgTs>
  requires(JointCollectionTo::template contains_joint_v<To> &&requires(
      JointCollectionFrom collection, ArgTs &&...value) {
  ...
  requires internal::is_joint_collection_castable<JointCollectionTo,
                                                  JointCollectionNext>
  auto as_reference() {
    return JointCollectionReference<JointCollectionFrom, JointCollectionNext>(
        ref_);
  }

  template <typename F>
  requires requires(const F &f) { JointCollectionTo::for_each(f); }
  static void for_each(const F &f) { JointCollectionTo::for_each(f); }
...
auto JointCollection<ChildLinkTs...>::as_reference() {
  return JointCollectionReference<JointCollection<ChildLinkTs...>,
                                  JointCollectionTo>(*this);
}
