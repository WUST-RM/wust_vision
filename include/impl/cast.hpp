...
template <typename T> struct get_depth {
  static constexpr int value = get_depth<typename Joint<T>::Parent>::value + 1;
};

template <> struct get_depth<Null> { static constexpr int value = 0; };

template <typename T> constexpr int get_depth_v = get_depth<T>::value;
...
constexpr auto get_lca(const T1 &link1, const T2 &link2) {
  if constexpr (std::is_same<T1, T2>::value)
    ...
    if constexpr (depth1 > depth2)
      ...
      return get_lca<depth1 - 1, depth2 - 1>(typename Joint<T1>::Parent{},
                                             typename Joint<T2>::Parent{});
  }
}

template <int, int> constexpr auto get_lca(const Null &, const auto &) {
  return Null{};
}

template <int, int> constexpr auto get_lca(const auto &, const Null &) {
  return Null{};
}

template <int, int> constexpr auto get_lca(const Null &, const Null &) {
  return Null{};
}

template <typename T1, typename T2> constexpr auto get_lca() {
...
requires(has_joint<LcaT, LinkT>) inline auto accumulate_transform(
    JointCollectionTs &...collections) {
  return get_transform<LcaT, LinkT>(collections...);
}

template <typename LcaT, typename LinkT, typename... JointCollectionTs>
inline auto accumulate_transform(JointCollectionTs &...collections) {
  return accumulate_transform<LcaT, typename Joint<LinkT>::Parent>(
             collections...) *
...
concept is_direction_vector_of_link =
    std::is_convertible_v<T,
                          typename Link<typename T::LinkType>::DirectionVector>;
template <typename T>
...
template <internal::is_link To, internal::is_position_of_link PositionT,
          typename... JointCollectionTs>
inline typename To::Position cast(const PositionT &position,
...
template <internal::is_link To, internal::is_direction_vector_of_link VectorT,
          typename... JointCollectionTs>
...
template <internal::is_link To, internal::is_rotation_of_link RotationT,
          typename... JointCollectionTs>
inline typename To::Rotation cast(const RotationT &rotation,
                                  const JointCollectionTs &...collections) {
...
auto lookup_transform(const JointCollectionTs &...collections) {
  if constexpr (std::is_same_v<From, To>) {
    ...
    if constexpr (std::is_same_v<Lca, From>) {
      ...
      return internal::accumulate_transform<Lca, From>(collections...)
          .inverse();
    } else {
      return internal::accumulate_transform<Lca, From>(collections...)
                 .inverse() *
