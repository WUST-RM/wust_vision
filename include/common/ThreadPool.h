...
public:
  explicit ThreadPool(size_t num_threads, size_t max_pending_tasks = 100,
      ...
      : stop_(false), max_pending_tasks_(max_pending_tasks),
  ...
  template <class F> void enqueue(F &&f, bool high_priority = false) {
    {
      std::unique_lock<std::mutex> lock(mutex_);
  ...
  void waitUntilEmpty() {
    std::unique_lock<std::mutex> lock(mutex_);
    task_done_cv_.wait(
        lock, [this]() { return tasks_.empty() && busy_workers_ == 0; });
  }
    ...
    while (true) {
      ...
      if (duration > max_task_duration_ms_) {
        WUST_WARN("ThreadPool")
            << "Warning: Task took too long: " << duration << " ms";
      }
...
inline void SetRealtimePriority(int priority = 90) {
  ...
  if (ret != 0) {
    WUST_ERROR("ThreadPool")
        << "Failed to set real-time priority. Error code: " << ret;
    perror("pthread_setschedparam");
  } else {
    WUST_INFO("ThreadPool")
        << "Real-time priority set successfully to " << priority;
  }
}
