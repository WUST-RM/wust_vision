...
struct SerialPortConfig {
  ...
  SerialPortConfig(unsigned int baud = 115200, unsigned int csize = 8,
      ...
      : baud_rate(baud), char_size(csize), parity(par), stop_bits(sb),
        flow_control(fc) {}
};
class SerialDriver {
public:
  ...
  void open() {
    port_.open(device_);
    port_.set_option(
        boost::asio::serial_port_base::baud_rate(config_.baud_rate));
    port_.set_option(
        boost::asio::serial_port_base::character_size(config_.char_size));
    port_.set_option(boost::asio::serial_port_base::parity(config_.parity));
    port_.set_option(
        boost::asio::serial_port_base::stop_bits(config_.stop_bits));
    port_.set_option(
        boost::asio::serial_port_base::flow_control(config_.flow_control));
  }
