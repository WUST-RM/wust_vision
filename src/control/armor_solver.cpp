...
Solver::Solver(const YAML::Node& config)
{
  init(config);
}
...
GimbalCmd Solver::solve(const Target& target, std::chrono::steady_clock::time_point current_time) {
  // 1. 获取最新的云台 RPY
  std::array<double,3> rpy{};
  Transform tf_gimbal;
  if (!tf_tree_.getTransform(target.frame_id,
                              "gimbal_link",
                              tf_gimbal))
  {
  ...
  auto armors = getArmorPositions(pos, yaw,
                                  target.radius_1,
                                  target.radius_2,
                                  target.d_zc,
                                  target.d_za,
                                  target.armors_num);
  int idx = selectBestArmor(armors, pos, yaw,
                            target.v_yaw,
                            target.armors_num);
  ...
  switch (state_) {
    case TRACKING_ARMOR:
      ...
      if ( controller_delay != 0.0) {
        ...
        auto tmp = getArmorPositions(pos, yaw,
                                     target.radius_1,
                                     target.radius_2,
                                     target.d_zc,
                                     target.d_za,
                                     target.armors_num).at(idx);
        if (tmp.norm() < 0.1) {
      ...
      fire_advice = isOnTarget(rpy[2], rpy[1],
                               raw_yaw, raw_pitch,
                               distance);
      break;
