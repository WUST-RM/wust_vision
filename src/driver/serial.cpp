...
void Serial::serialPortProtect()
{
    ...
    while (running_) {
      // 每次循环都打印一次状态
    //   WUST_INFO(serial_logger) << "[INFO] Serial port monitor (usb_ok=" << is_usb_ok_
    //             << "): " << device_name_ ;
      ...
      if (!is_usb_ok_) {
        try {
          ...
          is_usb_ok_ = false;
          WUST_ERROR(serial_logger) << "Re-open serial port failed: "
                    << ex.what() << "\n";
        }
      }
...
void Serial::receiveData()
{
    ...
    while (running_) {
        // 如果串口未就绪，等待并重试
        if (!is_usb_ok_) {
            WUST_WARN(serial_logger) << "eceive: usb is not ok! Retry count: "
                      << retry_count++ ;
            std::this_thread::sleep_for(
        ...
        try {
            ...
            if (!crc8::verify_CRC8_check_sum(
                   reinterpret_cast<uint8_t*>(&hf), sizeof(hf)))
            {
            ...
            while (remain > 0) {
                ...
                data_buf.insert(data_buf.begin() + total,
            ...
            data_buf.insert(data_buf.begin(),
                            header_buf.begin(), header_buf.end());
