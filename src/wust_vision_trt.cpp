...
void WustVision::init() {
  ...
  if (!tf_tree_.getTransform("camera_optical_frame", "gimbal_link", now, tf)) {
  ...
  Eigen::Matrix3d gimbal_to_camera_ = eigen_quat.toRotationMatrix();
  gimbal_to_camera_ = Sophus::SO3d::fitToSO3(gimbal_to_camera_).matrix();
  std::cout << gimbal_to_camera_ << std::endl;
