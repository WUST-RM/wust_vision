...
void drawGimbalDirection(cv::Mat& debug_img, const GimbalCmd& gimbal_cmd)
{
    ...
    dir_gimbal << 
        std::cos(gimbal_cmd.pitch) * std::sin(gimbal_cmd.yaw),  // X
    ...
    // 3. 投影到图像平面
    if (dir_cam.z() <= 0.01f) return; // 防止除零或背向相机

    float fx = static_cast<float>(measure_tool_->camera_intrinsic_.at<double>(0, 0));
    float fy = static_cast<float>(measure_tool_->camera_intrinsic_.at<double>(1, 1));
    float cx = static_cast<float>(measure_tool_->camera_intrinsic_.at<double>(0, 2));
    float cy = static_cast<float>(measure_tool_->camera_intrinsic_.at<double>(1, 2));
    ...
    for (auto &obj : objs) {
        for (size_t i = 0; i < 4; ++i) {
            ...
            if (obj.is_ok) {
                ...
                cv::putText(debug_img, fmt::format("{}", i), cv::Point2i(obj.pts_binary[i]), cv::FONT_HERSHEY_SIMPLEX, 0.8,cv::Scalar(255, 255, 0), 2);
            }
...
void drawresult(const imgframe &src_img, const Armors &armors)
{
    ...
    for (auto &armor : armors.armors) {
        ...
        if(!measure_tool_->reprojectArmorCorners_raw(armor, pts))continue;
        for (size_t i = 0; i < 4; ++i) {
            cv::line(debug_img, pts[i], pts[(i + 1) % 4], cv::Scalar(255, 100, 0), 2);    
           
        }
        std::string yaw_info = fmt::format("Yaw: {:.3f}", armor.yaw/M_PI * 180);
        cv::putText(debug_img, yaw_info, cv::Point(pts[0].x, pts[0].y-50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 200, 200), 2);
        ...
        cv::putText(debug_img, armor_info, cv::Point(pts[1].x, pts[1].y+50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 200, 200), 2);
   ...
   // yaw_diff = 0;
    ...
    cv::putText(debug_img, yaw_diff_info, cv::Point(100, 100), cv::FONT_HERSHEY_SIMPLEX, 2.7, cv::Scalar(40, 255, 40), 2);
    cv::circle(
      debug_img, cv::Point2i(1440 / 2., 1080 / 2.), 5,
      cv::Scalar(0, 0, 255), 1);

    auto latency_nano = std::chrono::duration_cast<std::chrono::nanoseconds>(now - armors.timestamp).count();
    double latency_ms = static_cast<double>(latency_nano) / 1e6;
...
void drawreprojec(const imgframe &src_img, const Target_info target_info,const Target target,const Tracker::State state,GimbalCmd gimbal_cmd)
{
    ...
    cv::circle(
      ...
      cv::Point armor_text_org(debug_img.cols - armor_text_size.width - 10,
                               state_text_org.y + state_text_size.height + 20);
      cv::putText(debug_img, armor_str, armor_text_org, cv::FONT_HERSHEY_SIMPLEX, 1.6, cv::Scalar(255, 0, 255), 2);
      std::string gimbal_info = fmt::format("Pitch: {:.2f}, Yaw: {:.2f}", gimbal_cmd.pitch, gimbal_cmd.yaw);
      std::string gimbal_diff_info = fmt::format("Pitch_Diff: {:.2f}, Yaw_Diff: {:.2f}", gimbal_cmd.pitch_diff, gimbal_cmd.yaw_diff);
       cv::putText(debug_img, gimbal_info, cv::Point(10, 90), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);
       cv::putText(debug_img, gimbal_diff_info, cv::Point(10, 120), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);
        if (gimbal_cmd.fire_advice) {
            std::string shoot_text = "SHOOT";
            int baseline = 0;
            cv::Size shoot_text_size = cv::getTextSize(shoot_text, cv::FONT_HERSHEY_DUPLEX, 3.5, 5, &baseline);
            cv::Point shoot_text_org(debug_img.cols - shoot_text_size.width - 20, 
                                     armor_text_org.y + armor_text_size.height + 40);
            cv::putText(debug_img, shoot_text, shoot_text_org, cv::FONT_HERSHEY_DUPLEX, 3.5,
                        cv::Scalar(0, 0, 255), 5);  // 红色加粗
        }
    ...
    const std::optional<GimbalCmd> &gimbal_cmd) 
{
        ...
        switch (armor.number) {
        case ArmorNumber::SENTRY: armor_str = "SENTRY"; break;
        case ArmorNumber::BASE: armor_str = "BASE"; break;
        case ArmorNumber::OUTPOST: armor_str = "OUTPOST"; break;
        case ArmorNumber::NO1: armor_str = "NO1"; break;
        case ArmorNumber::NO2: armor_str = "NO2"; break;
        case ArmorNumber::NO3: armor_str = "NO3"; break;
        case ArmorNumber::NO4: armor_str = "NO4"; break;
        case ArmorNumber::NO5: armor_str = "NO5"; break;
        default: armor_str = "UNKNOWN"; break;
        }
        ...
        double latency = std::chrono::duration<double, std::milli>(now - armors->timestamp).count();
        ...
        for (const auto &pt : all_corners) avg += pt;
        avg *= 1.0f / all_corners.size();
        cv::circle(debug_img, avg, 5, cv::Scalar(0, 255, 0), -1);
        }

        auto latency = std::chrono::duration_cast<std::chrono::microseconds>(now - target->timestamp).count() / 1000.0;
        auto latency_img_target = std::chrono::duration_cast<std::chrono::microseconds>(src_img.timestamp - target->timestamp).count() / 1000.0;

       

        cv::putText(debug_img, fmt::format("Img-Frame Delay: {:.2f}ms", latency_img_target),
        ...
        if (state.has_value()) {
            ...
            cv::Size state_size = cv::getTextSize(state_str, cv::FONT_HERSHEY_SIMPLEX, 2.5, 2, &baseline);
            int x = std::max(0, debug_img.cols - state_size.width - 10);
            int y = std::min(debug_img.rows - 1, state_size.height + 10); 
            cv::putText(debug_img, state_str, {x, y},
                        cv::FONT_HERSHEY_SIMPLEX, 2.5, cv::Scalar(0, 0, 255), 2);
        }
