...
bool AdaptedTRTModule::extractImage(const cv::Mat &src, ArmorObject &armor) {
  ...
  for (const auto &pt : armor.pts) {
    if (pt.x >= 0 && pt.x < src.cols && pt.y >= 0 && pt.y < src.rows &&
      ...
      // std::cerr << "[extractImage] Invalid pt detected and skipped: (" <<
      // pt.x << ", " << pt.y << ")" << std::endl;
    }
  }
  if (valid_pts.size() < 4) {
  ...
  cv::Mat litroi_bin;
  cv::threshold(litroi_gray, litroi_bin, binary_thres_, 255,
                cv::THRESH_BINARY | cv::THRESH_OTSU);
  armor.whole_binary_img = litroi_bin;

  // Perspective warp
  // 注意这里用 armor.pts 的原始点，如果不确定有效性，也可替换为 valid_pts
  // 对应点
  ...
  if (number_image.empty() || number_image.cols < roi_size.width ||
    ...
    //  std::cerr << "[extractImage] Warped number image is invalid" <<
    //  std::endl;
    return false;
  }

  // 截取 ROI 并处理
  cv::Rect number_roi((warp_width - roi_size.width) / 2, 0, roi_size.width,
                      roi_size.height);
  if ((number_roi.x + number_roi.width > number_image.cols) ||
      (number_roi.y + number_roi.height > number_image.rows)) {
    // std::cerr << "[extractImage] ROI for number image is out of bounds" <<
    // std::endl;
    return false;
  }
...
std::vector<Light> AdaptedTRTModule::findLights(const cv::Mat &rgb_img,
  ...
  std::sort(all_lights.begin(), all_lights.end(),
...
bool AdaptedTRTModule::isLight(const Light &light) noexcept {
  ...
  bool ratio_ok =
      light_params_.min_ratio < ratio && ratio < light_params_.max_ratio;
...
bool AdaptedTRTModule::classifyNumber(ArmorObject &armor) {
  ...
  minMaxLoc(outputs.reshape(1, 1), nullptr, &confidence, nullptr,
            &class_id_point);
...
std::vector<ArmorObject> AdaptedTRTModule::postprocess(
  ...
  for (int i = 0; i < num_detections; ++i) {
    ...
    // 解析颜色和类别
    obj.color = static_cast<ArmorColor>(
        std::max_element(det + 9, det + 9 + NUM_COLORS) - (det + 9));
    obj.number = static_cast<ArmorNumber>(
        std::max_element(det + 9 + NUM_COLORS,
                         det + 9 + NUM_COLORS + NUM_CLASSES) -
        (det + 9 + NUM_COLORS));
    // box.confidence = conf;
  ...
  // TopK
  std::sort(output_objs.begin(), output_objs.end(),
...
void AdaptedTRTModule::pushInput(
  ...
  thread_pool_->enqueue([=, this]() {
    // thread_local 分配独立的 context id
    thread_local int context_id = -1;
    if (context_id == -1) {
      context_id = next_context_id_++ % contexts_.size();
    }
