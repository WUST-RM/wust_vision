...
MonoMeasureTool::MonoMeasureTool(const std::string &yaml_path) {
  ...
  if (!setCameraInfo(camera_k, camera_d)) {
    WUST_ERROR(mono_logger)
        << "Failed to set camera info from YAML file: " << yaml_path;
  }
}
...
bool MonoMeasureTool::solvePnp(const std::vector<cv::Point2f> &points2d,
                               const std::vector<cv::Point3f> &points3d,
                               cv::Point3f &position, cv::Mat &rvec,
                               cv::SolvePnPMethod pnp_method) {
  if (points2d.size() != points3d.size()) {
    // WUST_WARN(mono_logger) << "2D-3D point size mismatch: " <<
    // points2d.size() << " vs " << points3d.size();
    return false;
  }
  ...
  cv::Mat r;
  bool res = cv::solvePnP(points3d, points2d, camera_intrinsic_,
                          camera_distortion_, r, trans, false, pnp_method);
...
// }
// }
bool MonoMeasureTool::calcArmorTarget(const ArmorObject &obj,
                                      cv::Point3f &position, cv::Mat &rvec,
                                      std::string &armor_type) {
  ...
  bool generic_ok = cv::solvePnPGeneric(
      *model_points, *image_points, camera_intrinsic_, camera_distortion_,
      rvecs, tvecs, false, /* useExtrinsicGuess */
      cv::SOLVEPNP_IPPE);
  ...
  for (size_t i = 0; i < rvecs.size(); ++i) {
    ...
    cv::projectPoints(*model_points, rvecs[i], tvecs[i], camera_intrinsic_,
                      camera_distortion_, reproj);
    double err = 0.0;
...
float MonoMeasureTool::calcDistanceToCenter(const ArmorObject &obj) {
  ...
  if (obj.is_ok) {
    armor_center.x = (obj.pts_binary[0].x + obj.pts_binary[1].x +
                      obj.pts_binary[2].x + obj.pts_binary[3].x) /
                     4.;
    armor_center.y = (obj.pts_binary[0].y + obj.pts_binary[1].y +
                      obj.pts_binary[2].y + obj.pts_binary[3].y) /
                     4.;
  } else {
    armor_center.x =
...
bool MonoMeasureTool::reprojectArmorCorners(
    const Armor &armor, std::vector<cv::Point2f> &image_points) {
  if (camera_intrinsic_.empty() || camera_distortion_.empty()) {
    WUST_ERROR(mono_logger) << "Camera parameters not initialized.";
    return false;
  }
  ...
  // 平移向量
  cv::Mat tvec = (cv::Mat_<double>(3, 1) << armor.target_pos.x,
                  armor.target_pos.y, armor.target_pos.z);

  // 反投影
  cv::projectPoints(*model_points, rvec, tvec, camera_intrinsic_,
                    camera_distortion_, image_points);
...
bool MonoMeasureTool::reprojectArmorCorners_raw(
  ...
  // 平移向量
  cv::Mat tvec =
      (cv::Mat_<double>(3, 1) << armor.pos.x, armor.pos.y, armor.pos.z);
...
bool MonoMeasureTool::reprojectArmorsCorners(Armors &armors,
...
void MonoMeasureTool::processDetectedArmors(
    const std::vector<ArmorObject> &objs, int detect_color,
    Armors &armors_out) {
  for (const auto &obj : objs) {
    ...
    try {
      ...
      armor.ori = {tf_quaternion.x, tf_quaternion.y, tf_quaternion.z,
                   tf_quaternion.w};
