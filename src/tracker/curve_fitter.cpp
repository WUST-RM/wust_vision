...
void CurveFitter::fitDoubleCurve() {
  ...
  std::for_each(
      data_history_queue_.begin(), data_history_queue_.end(),
      [&](const auto &data) {
        small_fitting_problem.AddResidualBlock(
            new ceres::AutoDiffCostFunction<CurveFitter::SmallRuneFittingCost,
                                            1, 3>(
        ...
        big_fitting_problem.AddResidualBlock(
            new ceres::AutoDiffCostFunction<CurveFitter::BigRuneFittingCost, 1,
                                            5>(
  ...
  big_fitting_problem.SetParameterLowerBound(big_param_ptr, 2,
                                             (2.090 - 1.045) * 0.5);
  big_fitting_problem.SetParameterUpperBound(big_param_ptr, 2,
                                             (2.090 - 0.780) * 1.5);
...
void CurveFitter::fitCurve() {
  ...
  // If the target is static, set the initial parameters, optimize the curve
  // from scratch
      ...
      [&](const auto &data) {
        if (type_ == MotionType::BIG) {
          problem.AddResidualBlock(
              new ceres::AutoDiffCostFunction<CurveFitter::BigRuneFittingCost,
                                              1, 5>(
                  new BigRuneFittingCost(data.time, data.angle, direction_)),
...
double CurveFitter::predict(double current_time) {
  ...
  if (type_ == MotionType::BIG) {
    pred_angle = BIG_RUNE_CURVE(
        current_time, fitting_param_[0], fitting_param_[1], fitting_param_[2],
        fitting_param_[3], fitting_param_[4], direction_);
  } else if (type_ == MotionType::SMALL) {
    pred_angle =
        SMALL_RUNE_CURVE(current_time, fitting_param_[0], fitting_param_[1],
                         fitting_param_[2], direction_);
  }
...
std::string CurveFitter::getDebugText() {
  ...
  if (type_ == MotionType::BIG) {
    ...
    t = fmt::format("V: {}( {:.2f} sin( {:.2f} (x {} {:.2f}) ) {} {:.2f} )",
                    direction_ == Direction::CLOCKWISE ? "-" : " ", a, omega,
                    (d > 0 ? "+" : "-"), std::abs(d), (b > 0 ? "+" : "-"),
                    std::abs(b));
...
void CurveFitter::update(double time, double angle) {
  ...
  double angle_diff =
      data_history_queue_.back().angle - data_history_queue_.front().angle;
  ...
  direction_ = static_cast<int>(angle_diff < 0 ? Direction::CLOCKWISE
  ...
  if (fitting_future_ == nullptr) {
    // First fitting
    startFitting();
    // Wait for the first fitting to finish in case the fitting is not finished
    // when the first prediction is needed
    fitting_future_->wait();
  } else if (fitting_future_->wait_for(std::chrono::seconds(0)) ==
